...
bool SettingsStore::from_json(const JsonObject& root) {
  ...
  settings_.spillover_min_on_min = constrain(
      root["spillover_min_on_min"] | settings_.spillover_min_on_min, 0, 5);
  ...
  // Never allow the low/high thresholds to overlap (or invert). Both were
  // independently clamped into [kFridgeControlMinC, kFridgeControlMaxC]
  // above, so it's possible for high_c to sit at or near its floor while
  // low_c sits above it. Resolve that by first pulling high_c up far enough
  // to keep the hysteresis gap (itself re-clamped to the legal range), then
  // deriving low_c from the (possibly capped) high_c. Doing it in this order
  // -- rather than a single constrain(low_c, min, high_c - hysteresis) --
  // avoids ever calling constrain() with an inverted (low > high) range,
  // which silently produces out-of-range results.
  if (settings_.high_c - settings_.low_c < hw::kHysteresisC) {
    settings_.high_c = constrain(settings_.low_c + hw::kHysteresisC,
                                 hw::kFridgeControlMinC,
                                 hw::kFridgeControlMaxC);
    settings_.low_c = constrain(settings_.high_c - hw::kHysteresisC,
                                hw::kFridgeControlMinC,
                                hw::kFridgeControlMaxC);
  }
  return true;
...
const String ConfigSchema(const SettingsStore&) {
  return R"JSON({
    ...
    "properties":{
      ...
      "freezer_lockout_c":{"title":"Freezer lockout temperature (C)","type":"number","minimum":-10,"maximum":10,"multipleOf":0.1},
      "fridge_alarm_c":{"title":"Fridge alarm temperature (C)","type":"number","minimum":3,"maximum":45,"multipleOf":0.1},
      "freezer_alarm_c":{"title":"Freezer alarm temperature (C)","type":"number","minimum":-15,"maximum":45,"multipleOf":0.1},
      "fan_delay_s":{"title":"Fan trigger delay (seconds)","type":"integer","minimum":5,"maximum":180},
      "spillover_min_on_min":{"title":"Spillover minimum ON (minutes)","type":"integer","minimum":0,"maximum":5},
