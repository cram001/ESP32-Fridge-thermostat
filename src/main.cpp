...
// kSettingCount, kLayoutSetting, kFirstAssignmentSetting, and
// kLastAssignmentSetting live in fridge_display.h so this file and
// fridge_display.cpp share one definition of the menu layout.
...
void update_encoder() {
  ...
  if (delta != 0) {
    ...
    if (assignment_mode && temperatures.detected_count() > 0) {
      const uint8_t options = temperatures.detected_count() + 1;
      assignment_sensor =
          (assignment_sensor + delta + options * 8) % options;
    } else if (!editing_setting) {
      selected_setting =
          (selected_setting + delta + kSettingCount * 8) % kSettingCount;
    ...
    } else if (selected_setting == 5 && editing_setting) {
      display_fahrenheit = !display_fahrenheit;
      setting_changed = true;
    } else if (selected_setting >= 6 && selected_setting <= 8 && editing_setting) {
      ...
      const float new_offset = constrain(shown_offset + delta * 0.1f,
                                        -shown_limit, shown_limit);
      if (new_offset != shown_offset) {
        setting_changed = true;
        calibration_c[role] = display_fahrenheit ? new_offset / 1.8f
                                                  : new_offset;
      }
    } else if (selected_setting == 9 && editing_setting) {
      const int new_value = constrain(
          static_cast<int>(settings.fan_delay_s) + delta * 5, 5, 180);
      setting_changed = new_value != settings.fan_delay_s;
      settings.fan_delay_s = new_value;
    } else if (selected_setting == 10 && editing_setting) {
      const int new_value = constrain(
          static_cast<int>(settings.spillover_min_on_min) + delta, 0, 5);
      setting_changed = new_value != settings.spillover_min_on_min;
      settings.spillover_min_on_min = new_value;
    } else if (selected_setting == 11 && editing_setting) {
      const int new_value = constrain(
          static_cast<int>(settings.circulation_min_on_min) + delta, 1, 5);
      setting_changed = new_value != settings.circulation_min_on_min;
      settings.circulation_min_on_min = new_value;
    } else if (selected_setting == 12 && editing_setting) {
      ...
      const uint8_t new_value = options[next_option];
      setting_changed = new_value != settings.emergency_spillover_on_min;
      settings.emergency_spillover_on_min = new_value;
    } else if (selected_setting == 13 && editing_setting) {
      ...
      selected_error = (selected_error + delta + faults.count() * 8) %
                       faults.count();
    } else if (selected_setting == 15 && editing_setting) {
      const int new_value = constrain(
          static_cast<int>(settings.oled_contrast_percent) + delta * 10,
          10, 100);
      setting_changed = new_value != settings.oled_contrast_percent;
      settings.oled_contrast_percent = new_value;
      display.set_contrast(settings.oled_contrast_percent);
    } else if (selected_setting == 16 && editing_setting) {
      ...
      const uint8_t new_value = options[next_option];
      setting_changed = new_value != settings.display_timeout_min;
      settings.display_timeout_min = new_value;
    } else if (selected_setting == kLayoutSetting && editing_setting) {
  ...
  if (button_down) {
    if (alarm_visual_active) {
      ...
      if (assignment_sensor >= temperatures.detected_count()) {
...
void update_display() {
  ...
  const float assignment_temp =
      (count > 0 && assignment_sensor < count)
...
void setup() {
  ...
  temperatures.begin(assigned_rom);
